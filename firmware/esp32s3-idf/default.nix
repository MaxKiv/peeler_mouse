...
let
  ...
  buildConfig = {
    ...
    environmentVariables = {
      ...
      CARGO_TARGET_XTENSA_ESP32S3_ESPIDF_RUNNER = "espflash flash --monitor";
...
in
{
  ...
  packages.esp32s3 =
    ...
    pkgs.rustPlatform.buildRustPackage {
      ...
      installPhase = ''
        mkdir -p $out
        cp target/${buildConfig.target}/release/esp32s3 $out
      '';
  ...
  apps.esp32s3 = {
    ...
    program = toString (
      pkgs.writeShellScript "flash-firmware" ''
        exec ${pkgs.lib.getExe pkgs.espflash} flash ${self.packages.${system}.esp32s3}/esp32s3
      ''
    );
  };
}
